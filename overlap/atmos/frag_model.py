...
def differential_equations(t, y, sigma_imp, rho_imp, eta):
    """
    Docstring
    """
...
def run_intergration(V0, M0, theta0, Z0, R0, Rdot0, sigma_imp, rho_imp, eta):
    """
    Docstring
    """
    ...
    event_dVdt_zero.terminal = True
    event_dVdt_zero.direction = 0

    event_pancake_with_R0.terminal = True
    event_pancake_with_R0.direction = -1

    events = [event_Z_crossing, event_mass_zero, event_dVdt_zero, event_pancake_with_R0]

    # Solve the differential equations
    sol_iso = solve_ivp(
        fun=lambda t, y: differential_equations(t, y, sigma_imp, rho_imp, eta),
        t_span=t_span,
    ...
    ram_pressure = 0.7 * 1.225 * np.exp(-altitude / 7.2e3) * vel ** 2 / 2
