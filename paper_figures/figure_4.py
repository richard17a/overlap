...
def main():
    ...
    for counter, D in enumerate(D_com):
        ...
        rho_f = (3 * mass[-1]) / (4 * np.pi * radius[-1] ** 3)

        if altitude[-1] < 1:

            D_f = calc_crater_diameter(2 * radius[-1], vel[-1], theta[-1], rho_f)
    ...
    for D in D_imp:
        ...
        D_f = calc_crater_diameter(D, 11.2e3, 45 * np.pi / 180, 0.6e3)
        D_f_com = np.append(D_f_com, D_f)
        D_f = calc_crater_diameter(D, 11.2e3, 45 * np.pi / 180, 3.5e3)
        D_f_ast = np.append(D_f_ast, D_f)
        D_f = calc_crater_diameter(D, 11.2e3, 45 * np.pi / 180, 7.8e3)
        D_f_iro = np.append(D_f_iro, D_f)

    fig_width, fig_height = set_size('thesis', 1, (1, 1))
    _ = plt.figure(figsize=(fig_width, fig_height))

    plt.plot(D_imp / 1e3, D_f_com / 1e3, color='tab:gray', ls='--')
    plt.plot(D_imp / 1e3, D_f_ast / 1e3, color='tab:gray', ls='--')
    plt.plot(D_imp / 1e3, D_f_iro / 1e3, color='tab:gray', ls='--')
